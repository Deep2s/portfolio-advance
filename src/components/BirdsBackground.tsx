...
export default function BirdsBackground() {
  ...
  useEffect(() => {
    ...
    let computeVelocity: any, computePosition: any, effectController: any;
    ...
    computeVelocity = Fn(() => {
      ...
      Loop({ start: uint(0), end: uint(BIRDS), type: "uint", condition: "<" }, ({ i }: any) => {
    ...
    const render = () => {
      ...
      pointer.y = 10; // Move pointer away so we only affect birds when moving the mouse
    };
